#!/usr/bin/env python3
"""
Python Code Typer - Windows Edition
...
def type_with_indentation(text: str):
    ...
    Accounts for LeetCode's auto-indentation on Enter (Windows optimized).
    """
    lines = text.split('\n')
    prev_char = None
    ...
    for i, line in enumerate(lines):
        ...
        if i > 0:
            ...
            # Windows: Use Home key to go to start of line
            pyautogui.press('home')
            time.sleep(0.05)
            
            # Delete any existing indentation (LeetCode auto-indents)
            # Windows: Use Delete key for forward delete
...
def main():
    """Main function."""
    print("Python Code Typer - Windows Edition (LeetCode)")
    print("=" * 50)
    ...
    if platform.system() != 'Windows':
        ...
        if response != 'y':
            print("Cancelled. Use pythonleecodetyper.py for Mac/Linux.")
            sys.exit(0)
